...
packages <- c(
  "quanteda", "quanteda.textmodels", "dplyr", "caret", "randomForest", "tm", "beepr", "rmarkdown", "e1071", "penalized", "plyr", "readr", "repr", "ggplot2", "rsample", "remotes", "stringr",
  "haven")

lapply(packages[!(packages %in% rownames(installed.packages()))], install.packages)

if(!("quanteda.classifiers" %in% rownames(installed.packages()))) remotes::install_github("quanteda/quanteda.classifiers")

lapply(c(packages, "quanteda.classifiers"), require, character.only = TRUE)
...
set.seed(300)
id_test <- sample(docvars(corp_press, "id"), round(length(docvars(corp_press, "id"))/5, 0), replace = FALSE)

# Create training and test set 
dfmat_training <- corpus_subset(corp_press, !(id %in% id_test)) %>%
  dfm(remove = stopwords("de"), stem = T, remove_punct = T, remove_number = T, remove_symbols = T, remove_url = T) %>% # stem and remove stopwords, punctuation etc.
  dfm_trim(min_docfreq = 0.005, max_docfreq = .8, docfreq_type = "prop") # Remove words occurring in less than .5% or more than 80% of documents
