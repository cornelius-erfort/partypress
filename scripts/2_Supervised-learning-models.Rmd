...
for (country in press$country %>% unique) {
  ...
  print(model_id)
   if(!(str_c(country, model_id) %in% str_c(tm_eval$country, tm_eval$model_id))) {
    ...
    #   eval_start <- Sys.time()
    #   randomforest_eval <-  randomForest(x = as.matrix(dfm_subset(dfmat, dfmat$cv_sample != 1 & (dfmat$issue %in% dfmat$issue[dfmat$cv_sample == 1]))), y = as.factor(dfm_subset(dfmat, dfmat$cv_sample != 1 & (dfmat$issue %in% dfmat$issue[dfmat$cv_sample == 1]))$issue),
    ...
    # 
    # 
    #  model_eval <- data.frame(accuracy = accuracy(randomforest_pred, randomforest_test$issue),
    #            time = as.numeric(eval_end - eval_start),
    #            seed = seed,
    #            weight = "uniform",
    #            model_name = model_name,
    ...
    #            country = country)
    # 
    #     add_param <- cbind(quanteda.classifiers::precision(randomforest_pred, randomforest_test$issue) %>% as.data.frame(),
    #                        quanteda.classifiers::recall(randomforest_pred, randomforest_test$issue) %>% as.data.frame(),
    #                        quanteda.classifiers::f1_score(randomforest_pred, randomforest_test$issue)  %>% as.data.frame() %>% mutate(class = rownames(.)))  %>% mutate(country = country)
    # 
    #  tm_eval <- merge(model_eval, add_param, by = "country") %>% rbind.fill(tm_eval)
    # 
    # }
    ...
    #   eval_start <- Sys.time()
    #    if(!(str_c(country, model_id) %in% str_c(tm_eval$country, tm_eval$model_id))) {
    #      randomforest_eval_alt <-  randomForest(x = as.matrix(dfm_subset(dfmat_alt, dfmat_alt$cv_sample != 1 & (dfmat$issue %in% dfmat$issue[dfmat$cv_sample == 1]))), y = as.factor(dfm_subset(dfmat_alt, dfmat_alt$cv_sample != 1 & (dfmat$issue %in% dfmat$issue[dfmat$cv_sample == 1]))$issue),
    ...
    # randomforest_test_alt <- dfm_subset(dfmat_alt, dfmat_alt$cv_sample == 1)
    # 
    #   model_eval <- data.frame(accuracy = accuracy(randomforest_pred_alt, randomforest_test_alt$issue),
    #            time = as.numeric(eval_end - eval_start),
    #            seed = seed,
    #            weight = "tfidf",
    #            model_name = model_name,
    ...
    #            country = country)
    # 
    #     add_param <- cbind(quanteda.classifiers::precision(randomforest_pred_alt, randomforest_test_alt$issue) %>% as.data.frame(),
    #                        quanteda.classifiers::recall(randomforest_pred_alt, randomforest_test_alt$issue) %>% as.data.frame(),
    #                        quanteda.classifiers::f1_score(randomforest_pred_alt, randomforest_test_alt$issue)  %>% as.data.frame() %>% mutate(class = rownames(.)))  %>% mutate(country = country)
    # 
    #  tm_eval <- merge(model_eval, add_param, by = "country") %>% rbind.fill(tm_eval)
    # 
    # }
...
# if(!dir.exists("files/supervised")) dir.create("files/supervised")
