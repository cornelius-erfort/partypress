...
load("other-countries/textpress.RData")
corp_press <- corpus(textpress$htext,
                       docvars = select(textpress, c(country, issue, party, cv_sample)))
dfmat_alt <- corpus_subset(corp_press, country == "spain" & !is.na(issue)) %>%
    tokens() %>% tokens_ngrams(n = 1:2) %>%
    dfm(remove = stopwords("es"), # Stem and remove stopwords, punctuation etc.
        remove_punct = T, remove_number = T, remove_symbols = T, remove_url =   T) %>% dfm_wordstem(language = "es") %>%
...
trainsize_ridge$accuracy <- sapply(trainsize_ridge$train_size, FUN = function (x) {
  sub_train <- sample(1:ndoc(dfm_subset(dfmat_alt, dfmat_alt$cv_sample != 1)), x)
  ridge_opt_mod <- textmodel_svm(dfm_subset(dfmat_alt, dfmat_alt$cv_sample != 1)[sub_train, ], dfm_subset(dfmat_alt, dfmat_alt$cv_sample != 1)$issue[sub_train], type = 7)
  accuracy(dfm_subset(dfmat_alt, dfmat_alt$cv_sample == 1)$issue, predict(ridge_opt_mod, newdata = dfm_subset(dfmat_alt, dfmat_alt$cv_sample == 1)))}) %>% unlist

# Load the results from BETO transformer models
trainsize_transfer <- read_csv("other-countries/results-training-size-beto.csv", col_names = F)
names(trainsize_transfer) <- c("accuracy", "train_size")
trainsize_transfer$Model <- "Transformers"
train_size <- rbind(trainsize_transfer, trainsize_ridge)

# Load the results from multilingual transformer models
trainsize_multi <- read_csv("other-countries/results-training-size-multilingual.csv", col_names = F)
names(trainsize_multi) <- c("accuracy", "train_size")
trainsize_multi$Model <- "Transformers (multilingual)"
