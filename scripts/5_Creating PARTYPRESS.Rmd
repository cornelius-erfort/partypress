...
save(partypress, file =  "partypress.RData")
write.csv(partypress, file = "partypress.csv")
...
save(partypress_issues, file = "partypress_issues.RData")
write.csv(partypress_issues, file = "partypress_issues.csv")
...
save(partypress_texts, file = "partypress_texts.RData")
write_csv(partypress_texts, file = "partypress_texts.csv")
...
monthly_agendas <- partypress %>% select(c(country_name, month, issue_multi, party, parlgov_id))

# Add variable for counting
monthly_agendas$freq <- 1

# Aggregate by party, date and issue
monthly_agendas <- aggregate(freq ~ country_name + party + parlgov_id + month + issue_multi, monthly_agendas, sum)

# Add observations with zero documents
for (thisparty in unique(monthly_agendas$parlgov_id)) {
  for(thismonth in unique(monthly_agendas$month[monthly_agendas$parlgov_id == thisparty])) {
    for(thisissue in unique(monthly_agendas$issue_multi)) {
      if(nrow(monthly_agendas[monthly_agendas$parlgov_id == thisparty & monthly_agendas$month == thismonth & monthly_agendas$issue_multi == thisissue, ]) == 0 & nrow(monthly_agendas[monthly_agendas$parlgov_id == thisparty & monthly_agendas$month == thismonth, ]) != 0) {
        monthly_agendas <- data.frame(
          parlgov_id = thisparty, month = thismonth, issue_multi = thisissue, freq = 0
          ) %>% rbind.fill(monthly_agendas)
}}}}

# Add var for total press releases per party and month
monthly_agendas$party_sum <- ave(monthly_agendas$freq, monthly_agendas$month, monthly_agendas$parlgov_id, FUN = sum)
...
save(monthly_agendas, file = "monthly_agendas.RData")
write.csv(monthly_agendas, "monthly_agendas.csv")
...
save(weekly_agendas, file = "weekly_agendas.RData")
write.csv(weekly_agendas, "weekly_agendas.csv")
