...
saveRDS(partypress_issues, file = "publication/partypress_issues.RDS")
write.csv(partypress_issues, file = "publication/partypress_issues.csv", row.names = F, fileEncoding = "UTF-8")
write_dta(partypress_issues, path = "publication/partypress_issues.dta")
...
monthly_agendas <- monthly_agendas %>% dplyr::group_by(parlgov_id, month_start) %>% dplyr::mutate(party_sum = sum(freq, na.rm = T)) %>% filter(party_sum > 0) 
...
monthly_agendas <- merge(monthly_agendas, issue_categories, by = "issue") %>% select(-c(freq))

# Fill vars
monthly_agendas <- monthly_agendas %>% dplyr::group_by(parlgov_id) %>% do(fill(., country_name, party, party_name, party_name_english, family_name))
...
monthly_agendas <- select(monthly_agendas, c(country_name, parlgov_id, party, party_name, party_name_english, family_name, month, month_start, month_end, issue, model, issue_descr, share, party_sum))
...
# Add issue descriptions
weekly_agendas <- merge(weekly_agendas, issue_categories, by = "issue") %>% select(-c(freq))
...
weekly_agendas <- select(weekly_agendas, c(country_name, parlgov_id, party, party_name, party_name_english, family_name, calendar_week, week_start, week_end, issue, model, issue_descr, share, party_sum))
