...
packages <- c(
  "quanteda", "quanteda.textplots", "quanteda.textmodels", "quanteda.textstats", "dplyr", "tm", "rmarkdown", "plyr", "readr", "ggplot2", "stringr", "formatR", "readstata13", "lubridate", "glmnet", "kableExtra", "stargazer")
...
issue_agendas <- merge(issue_agendas_supervised %>% dplyr::rename(attention_supervised = attention), issue_agendas_readme %>% dplyr::rename(attention_readme = attention), by = c("party", "issue_r1", "date"), all = T)
...
plot_issue <- 7
ggplot(issue_agendas_readme %>% filter(issue_r1 %in% plot_issue), aes(x = date, y = attention)) +
  geom_step(alpha = .4) +
  geom_step(data = issue_agendas %>% filter(issue_r1 %in% plot_issue), color = "red", alpha = .4) + 
    geom_smooth(data = issue_agendas %>% filter(issue_r1 %in% plot_issue), method = "loess", formula = "y ~ x", color = "pink", lty = 2, se = F) +
  geom_smooth(method = "loess", formula = "y ~ x", color = "dark grey", lty = 2, se = F) +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  # ylim(c(0., max(issue_agendas_readme$attention) + 0.05)) +
  ylab("Share of press releases per quarter") +
  facet_wrap(~ party) +
  ggsave(str_c("plots/", plot_issue, " - ", 
               issue_categories$issue_r1_descr[issue_categories$issue_r1 == plot_issue],"_comparing methods.pdf"), 
         device = cairo_pdf, width = 5*2^.5, height = 5) +
  ggsave(str_c("plots/", plot_issue, " - ", 
               issue_categories$issue_r1_descr[issue_categories$issue_r1 == plot_issue],"_comparing methods.png"), 
...
ggplot(agg_eval, aes(x = truth, y = predicted)) +
  ...
  # geom_text(label = agg_eval$issue_r1 %>% 
...
ggplot(agg_eval, aes(x = truth, y = predicted)) +
  ...
  geom_text_repel(label = agg_eval$issue_r1 %>%
              str_extract("[:digit:]{1,2}(\\.[:digit:])?"),
              box.padding = .4,
            color = "dark grey", size = 1.5, family = "LM Roman 10", 
            segment.size = .25, 
