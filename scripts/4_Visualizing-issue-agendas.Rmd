...
spec_att$time <- ""
spec_att$time[spec_att$date >= ymd("2015-01-01") & spec_att$date <= ymd("2015-12-31")] <- "2015" 
spec_att$time[spec_att$date >= ymd("2011-01-01") & spec_att$date <= ymd("2011-12-31")] <- "2011" 
...
ggplot(spec_att, aes(x = time, y = attention)) +
  ...
  geom_text_repel(data = spec_att[spec_att$time %in% c("2011", "2013"), ], aes(x = spec_att[spec_att$time %in% c("2011", "2013"), "time"], y = spec_att[spec_att$time %in% c("2011", "2013"), "attention"]), label = round(spec_att[spec_att$time %in% c("2011", "2013"), "attention"], 2), hjust = "left", colour = "dark grey", family = "LM Roman 10", box.padding = .4, size = 4, segment.size = .25, 
            min.segment.length = .1,
            point.padding = .5, max.overlaps = 100, nudge_x = -.25) +
  geom_text_repel(data = spec_att[!(spec_att$time %in% c("2011", "2013")), ], aes(x = spec_att[!(spec_att$time %in% c("2011", "2013")), "time"], y = spec_att[!(spec_att$time %in% c("2011", "2013")), "attention"]), label = round(spec_att[!(spec_att$time %in% c("2011", "2013")), "attention"], 2), hjust = "right", colour = "dark grey", family = "LM Roman 10", box.padding = .4, size = 4, segment.size = .25, 
            min.segment.length = .1,
            point.padding = .5, max.overlaps = 100, nudge_x = .25) +
