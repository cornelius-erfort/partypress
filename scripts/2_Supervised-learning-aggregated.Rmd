...
issue_eval <- data.frame(issue_r1 = dfmat_test$issue_r1, predicted = predict(baseline_nb, newdata = dfmat_test)) %>% 
  ...
  merge(., data.frame(issue_r1 = table(dfmat_test$issue_r1) %>% rownames(),
           ...
           f1_score = f1_score(dfmat_test$issue_r1, predict(baseline_nb, newdata = dfmat_test))), by = "issue_r1") %>% dplyr::rename(f1_score = f1, issue = issue_r1)
issue_eval[, c("accuracy", "precision", "recall", "f1_score")] <- issue_eval[, c("accuracy", "precision", "recall", "f1_score")] %>% apply(., MARGIN = 2, FUN = function (x) round(as.numeric(x), 2))

# Change and order labels
issue_eval$issue[issue_eval$issue == 191] <- 19.1
issue_eval$issue[issue_eval$issue == 192] <- 19.2
issue_eval$issue <- as.factor(as.numeric(issue_eval$issue))
levels(issue_eval$issue) <- str_c(levels(issue_eval$issue), " - ", issue_categories[c(1:13, 16:17, 14:15), 2])
