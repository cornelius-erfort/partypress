...
#########
dfmat_test$issue_r1 %>% table

truth <- dfmat_test$issue_r1
truth[truth == 191] <- 19.1
truth[truth == 192] <- 19.2
truth <- factor(truth, levels = c(1:7, 9:10, 12, 15:17, 19.1, 19.2, 20, 99))

prediction <-predict(baseline_nb, newdata = dfmat_test) %>% as.numeric()
...
latex_out <- capture.output(as.data.frame(unclass((table(prediction, truth) %>%
 ...
 %>%
  stargazer(summary = F,
            title = "Confusion matrix for the test data (naive Bayes)", 
            label = "tab:confusion-mat-nb"))

latex_out <- capture.output(latex_out %>% str_replace_all( "tabular", "tabularx")  %>% str_replace_all("\\@\\{\\\\extracolsep\\{5pt\\}\\} cccccccccccccccccc", "\\\\textwidth\\}\\{XXXXXXXXXXXXXXXXXX") %>% cat(sep = "\n"), file = "tables/confusion-mat-nb.tex")




issue_eval <- data.frame(issue_r1 = dfmat_test$issue_r1, predicted = predict(baseline_nb, newdata = dfmat_test)) %>% 
  group_by(issue_r1) %>% 
  dplyr::mutate(accuracy = accuracy(issue_r1, predicted)) %>% select(-c("predicted")) %>% unique() %>%
  merge(., data.frame(issue_r1 = table(dfmat_test$issue_r1) %>% rownames(),
           precision = precision(dfmat_test$issue_r1, predict(baseline_nb, newdata = dfmat_test)),
           recall = recall(dfmat_test$issue_r1, predict(baseline_nb, newdata = dfmat_test)),
           f1_score = f1_score(dfmat_test$issue_r1, predict(baseline_nb, newdata = dfmat_test))), by = "issue_r1") %>% 
  dplyr::rename("Accuracy" =  accuracy, "Precision" = precision, "Recall" = recall, "F1 score" = f1, Issue = issue_r1)
issue_eval[, c("Accuracy", "Precision", "Recall", "F1 score")] <- issue_eval[, c("Accuracy", "Precision", "Recall", "F1 score")] %>% apply(., MARGIN = 2, FUN = function (x) round(as.numeric(x), 3))

# Change and order labels
issue_eval$Issue[issue_eval$Issue == 191] <- 19.1
issue_eval$Issue[issue_eval$Issue == 192] <- 19.2
issue_eval$Issue <- str_c(as.character(issue_eval$Issue), " - ", issue_categories[, 2])
issue_eval <- issue_eval[c(1:13, 16:17, 14:15), ]

# Write latex table
if(!dir.exists("tables")) dir.create("tables")
latex_out <- capture.output(issue_eval %>%
  stargazer(out = "tables/issue-eval-nb.tex", summary = F, rownames = F, 
            title = "Performance statistics by issue (Naive Bayes)", 
...
# all_germany <- read_rds("data/data_joint.RDS") %>% select(c(header, text.x, date.x, issue, party.x, id, filepath, filename)) %>% filter(!is.na(date.x) & !is.na(text.x) & (issue != "98 Non-thematic" | is.na(issue))) %>% dplyr::rename(party = party.x, date = date.x)
