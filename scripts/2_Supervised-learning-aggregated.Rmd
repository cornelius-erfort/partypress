...
party_names <- data.frame(party = c("90gruene_fraktion",
                          ...
                          party_name = c("Bündnis 90/Die Grünen", 
                                         "AfD", "AfD", 
                                         "FDP", "FDP", "DIE LINKE", 
                                         "SPD", "CDU/CSU"))
...
latex_out <- capture.output(
  ...
  select(docvars(dfmat_all), c("party_name", "date", "header"))[order(t(dfmat_all$issue_r1_prob)[colnames(dfmat_all$issue_r1_prob) == "7"], decreasing = T), ][1:1000, ][sample(1:100, 10), ] %>% dplyr::rename(party = party_name, title = header) %>%
  stargazer(summary = F, rownames = F, 
            title = "Sample of press releases classified as category 7 - Environment and Energy", label = "tab:7-document-samples") %>% str_replace_all( "tabular", "tabularx") %>% str_replace_all("\\@\\{\\\\extracolsep\\{5pt\\}\\} ccc", "\\\\textwidth") %>% cat(sep = "\n")
...
myx <- select(docvars(dfmat_all), c("party_name", "date", "header"))[order(t(dfmat_all$issue_r1_prob)[colnames(dfmat_all$issue_r1_prob) == "7"], decreasing = T), ][1:1000, ][sample(1:100, 10), ] %>% dplyr::rename(party = party_name, title = header) %>% 
  xtable(title = "Sample of press releases classified as category 7 - Environment and Energy", label = "tab:7-document-samples")
print.xtable(myx, tabular.enivronment = "tabularx", width = "\\textwidth", floating.environment = F)
...
latex_out <- capture.output(select(docvars(dfmat_all), c("party_name", "date", "header"))[order(t(dfmat_all$issue_r1_prob)[colnames(dfmat_all$issue_r1_prob) == "9"], decreasing = T), ][1:1000, ][sample(1:100, 10), ] %>% dplyr::rename(party = party_name, title = header) %>%
  resizebox.stargazer(tab.width = "\\textwidth", summary = F, rownames = F, 
            title = "Sample of press releases classified as category 9 - Immigratioon", label = "tab:7-document-samples"), file = "tables/9-document-samples.tex")
