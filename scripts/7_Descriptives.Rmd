...
ggplot(press) +
  geom_boxplot(aes(x = country %>% str_to_title() %>% str_replace("Uk", "UK"), y = str_length), outlier.shape = NA) + 
...
table(press$year, press$party_name, press$country) %>% as.data.frame() %>% 
  ...
  ggsave("coverage_party_year.pdf", device = cairo_pdf, width = 10, height = 8*2^.5)
  
  
  table(press$year, press$country) %>% as.data.frame() %>% 
  ...
  ggsave("coverage_country_year.pdf", device = cairo_pdf, width = 5*2^.5, height = 5)
...
# Immigration
sample9 <- select(partypress, c("country", "party_name", "date", "header", "multi")) %>% filter(multi == 9)
