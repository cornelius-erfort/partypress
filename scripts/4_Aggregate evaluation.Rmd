...
agg_eval <- multi_agg # rbind.fill(multi_agg, cross_agg)


# Transformer monolingual

if(file.exists("files/mono_pred.RData"))  load("files/mono_pred.RData") else {
  mono_pred <- read_csv("files/monolingual-prediction.csv", col_names = c("country", "cv_sample", "label", "prediction", "id")) %>% dplyr::rename("pred_label" = "prediction") %>% 
  ...
  mono_pred$issue %>% table
  mono_pred$issue <- str_replace_all(mono_pred$issue, c("191" = "19.1", "192" = "19.2")) %>% factor(levels = c(1:10, 12:18, 19.1, 19.2, 20, 23, 98, 99))
  mono_pred$prediction <- str_replace_all(mono_pred$prediction, c("191" = "19.1", "192" = "19.2")) %>% factor(levels = c(1:10, 12:18, 19.1, 19.2, 20, 23, 98, 99))
...
# MSE
mse <- agg_eval %>% group_by(model) %>% dplyr::summarise(MSE = sum((Difference*100)^2)/n(), 
                                                  RMSE = (sum((Difference*100)^2)/n())^.5, n = mean(n)) %>% arrange(MSE)
