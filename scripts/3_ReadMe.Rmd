...
if(!("wordVec_summaries_labeled" %in% ls())) if(file.exists("readme-files/wordvectors/wordVec_summaries_labeled.RData")) {
  ...
  ## Generate a word vector summary for all documents
    ...
    cleaned_text <- cleanme(alldocs$htext)
    glove_ger_mat <- glove_ger %>% as.matrix
    
    wordVec_summaries_labeled <- undergrad(
    documentText = cleaned_text,
    wordVecs = glove_ger_mat)
...
for (i in unique(alldocs$cv_sample) %>% sort) {
  # Estimate category proportions
  start_readme <- Sys.time()
  readme_cv[[i]] <- readme(
    dfm = wordVec_summaries_labeled, 
    labeledIndicator = ifelse(alldocs$cv_sample == i, 0, 1),
    categoryVec = alldocs$issue,
    nCores = 16,
...
agg_eval$issue[agg_eval$issue == 191] <- 19.1
agg_eval$issue[agg_eval$issue == 192] <- 19.2
agg_eval$issue <- as.factor(as.numeric(agg_eval$issue))
levels(agg_eval$issue) <- str_c(levels(agg_eval$issue), " - ", issue_categories[c(1:13, 16:17, 14:15), 2])

save(agg_eval, file = "readme-files/agg_eval.RData")

# Write latex table
if(!dir.exists("tables")) dir.create("tables")
# latex_out <- capture.output(stargazer(agg_eval %>% dplyr::group_by(issue) %>% 
...
plot_agg_eval(agg_eval %>% dplyr::group_by(issue) %>% 
  dplyr::summarise(predicted = mean (predicted),
...
# Unite parties
all_germany$party <- all_germany$party %>% str_replace_all(c("union_fraktion" = "CDU/CSU", "spd_fraktion" = "SPD", "90gruene_fraktion" = "B'90/Die Grünen", "fdp_bundesverband" = "FDP", "fdp_fraktion" = "FDP", "linke_fraktion" = "DIE LINKE", "afd_bundesverband" = "AfD", "afd_fraktion" = "AfD"))

# Merge categories
all_germany$issue <- all_germany$issue %>% as.character %>% str_replace_all(c("a" = "1", "b" = "2")) %>% str_extract("[:digit:]*") %>% as.numeric
...
for (party in unique(all_germany$party)) {
  ...
  load ("readme-files/wordvectors/wordVec_summaries_all.RData") }
    ...
    readme_storage <- readme(
      dfm = wordVec_summaries_all[(all_germany$party == party &   all_germany$date == date) | !is.na(all_germany$issue), ],
      labeledIndicator =   ifelse(is.na(all_germany$issue)[(all_germany$party == party &   all_germany$date == date) | !is.na(all_germany$issue)], 0, 1),
      categoryVec = all_germany$issue[(all_germany$party ==   party & all_germany$date == date) | !is.na(all_germany$issue)],
      nCores = 2,
